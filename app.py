...
def generate_stamp(previous_value: Dict) -> Dict:
    """
    Генерирует следующий штамп на основе предыдущего.

    Args:
        previous_value: Предыдущий штамп (словарь с ключами offset и score)

    Returns:
        Новый штамп с увеличенным offset и возможно изменённым счётом
    """
    # Определяем, изменился ли счёт
    score_changed = random.random() > 1 - PROBABILITY_SCORE_CHANGED

    # Изменение счёта домашней команды
    if score_changed and random.random() > 1 - PROBABILITY_HOME_SCORE:
        home_score_change = 1
        away_score_change = 0
    else:
        home_score_change = 0
        away_score_change = 1 if score_changed else 0

    # Шаг изменения offset
...
def generate_game() -> List[Dict]:
    """
    Генерирует полную последовательность штампов для игры.

    Returns:
        Список штампов, начиная с INITIAL_STAMP и добавляя новые
    """
...
def get_score(game_stamps: List[Dict], offset: int) -> Tuple[int, int]:
    """
    Возвращает счёт на момент указанного offset.

    Args:
        game_stamps: Список штампов игры, отсортированный по offset
        offset: Временная метка, для которой нужно получить счёт

    Returns:
        Кортеж (home_score, away_score) на момент offset

    Особенности:
        - Если offset меньше 0, возвращает начальный счёт (0, 0)
        - Если offset превышает максимальный в списке, возвращает последний известный счёт
...
if __name__ == "__main__":
    ...
    print(
        f"Финальный счёт: Домашние {game_stamps[-1]['score']['home']} - "
