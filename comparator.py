...
class RouteComparator:
    ...
    def __init__(self, file1_itineraries: List[Dict], file2_itineraries: List[Dict]) -> None:
        """Инициализация компаратора с двумя списками маршрутов."""
    ...
    def compare(self) -> Dict:
        ...
        file1_routes = {itinerary['route_key']: itinerary for itinerary in self.file1_itineraries}
        file2_routes = {itinerary['route_key']: itinerary for itinerary in self.file2_itineraries}
            ...
            'removed_routes': [
                {
                    ...
                    'start_time': file1_routes[key]['start_time'].strftime(self.DATE_FORMAT),
                    'end_time': file1_routes[key]['end_time'].strftime(self.DATE_FORMAT),
            ...
            'new_routes': [
                {
                    ...
                    'start_time': file2_routes[key]['start_time'].strftime(self.DATE_FORMAT),
                    'end_time': file2_routes[key]['end_time'].strftime(self.DATE_FORMAT),
    ...
    def _compare_single_route(self, route1: Dict, route2: Dict) -> Dict:
        """Сравнить два маршрута и вернуть словарь с изменениями."""
        changes = {}

        # Сравнение времени начала
        if route1['start_time'] != route2['start_time']:
            changes['start_time'] = {
                'old': route1['start_time'].strftime(self.DATE_FORMAT),
                'new': route2['start_time'].strftime(self.DATE_FORMAT)
            }

        # Сравнение времени окончания
        if route1['end_time'] != route2['end_time']:
            changes['end_time'] = {
                'old': route1['end_time'].strftime(self.DATE_FORMAT),
                'new': route2['end_time'].strftime(self.DATE_FORMAT)
            }
        ...
        if route1['total_amount'] != route2['total_amount']:
            changes['price'] = {
                ...
                'difference': (
                    route2['total_amount'] - route1['total_amount']
                    if route1['total_amount'] is not None and route2['total_amount'] is not None
                    else None
                )
            }
    ...
    def print_comparison_results(self) -> None:
        """Вывести результаты сравнения в консоль."""
    ...
    def save_results_to_excel(self, filename: str = 'comparison_results.xlsx') -> None:
        """Сохранить результаты сравнения в Excel-файл."""
        results = self.comparison_results
